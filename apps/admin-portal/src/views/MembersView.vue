  ...
  <AppLayout>
    <div class="flex items-center justify-between mb-6">
      <div>
        ...
        <h1 class="text-2xl md:text-3xl font-black tracking-tight text-gray-900">會員管理</h1>
      </div>
      <RouterLink to="/members/new" class="btn-primary">+ 新增</RouterLink>
    </div>
    ...
    <div class="card overflow-hidden">
      ...
      <template v-else-if="members.length === 0">
        <div class="text-center py-16 text-gray-300">尚無會員資料</div>
      </template>

      <template v-else>
        <!-- Mobile card list -->
        <div class="md:hidden divide-y divide-gray-50">
          <RouterLink v-for="m in members" :key="m.sub" :to="`/members/${m.sub}`"
            class="flex items-center gap-3 px-5 py-4 hover:bg-gray-50/60 transition-colors">
            <div class="flex-1 min-w-0">
              <p class="font-semibold text-gray-900 text-sm truncate">{{ m.name }}</p>
              <p class="text-xs text-gray-400 mt-0.5 truncate">{{ m.email }}</p>
            </div>
            <div class="flex items-center gap-2.5 shrink-0">
              <span class="inline-flex items-center gap-1 text-xs font-semibold px-2 py-0.5 rounded-full"
                :class="m.status === 'active' ? 'bg-emerald-50 text-emerald-700' : 'bg-red-50 text-red-600'">
